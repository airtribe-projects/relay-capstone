...
- [ ] Load the node catalog and the four seed workflows as Published version 1.
- [ ] Create, list, publish workflows in the seed JSON shape; publish validation rejects broken definitions.
- [ ] Trigger runs via webhook (with `X-Relay-Secret`), schedule (cron), and manual API.
...
- [ ] Runs execute on a worker via a durable queue; the API never runs a workflow inline.
...
- [ ] Per-node retries with backoff; run caps (max_steps, timeout, token budget) stop `wf_runaway`.
- [ ] `POST /workflows/compile` turns plain English into valid drafts and refuses the traps.
- [ ] One-command NL eval reports per-case results and accuracy over `data/nl_eval.jsonl`.
...
- Implement create/list/get, publish with validation against the catalog, and versioning.
- Validation errors are worth real effort here — the smoke test checks three specific rejection cases, and your NL compiler will reuse this validator in Step 8.
...
- Run caps; verify with `wf_runaway`.
- Webhook and cron triggers if you have not added them yet.
...
- Record token usage per step and enforce the run's token cap.
...
- Smoke test green; two kill-and-resume drills with duplication check output; NL eval accuracy report; automated tests for validation, templates, schema enforcement, idempotency.
...
### Scenario 3: Compile from English
...
For `ai` nodes and the NL compiler in demos and the eval: yes, but free-tier (Gemini, Groq) or local (Ollama) is fine. For engine tests: no — the adapter can return canned JSON, and `scripts/mock_provider.py` exists if you want an HTTP-level fake.
...
### Does the NL compiler have to produce perfect workflows?
...
### How precise do schedules need to be?
...
- API overview,
- how to run the smoke test, the drill, and your NL eval,
- architecture/design decisions (run state machine, queue choice, idempotency scheme, compiler design),
- known limitations.
